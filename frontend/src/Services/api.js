const getReferrer = () => {
  ...
  if (typeof import.meta !== "undefined" && import.meta.env) {
    ...
    return (
      import.meta.env.VITE_WEBSITE_URL ||
      "https://studybuddy-one-chi.vercel.app/"
    );
  }
...
export const askAI = async (messages) => {
  ...
  const API_KEY =
    typeof import.meta !== "undefined" && import.meta.env
      ? import.meta.env.REACT_APP_OPENROUTER_KEY ||
        import.meta.env.VITE_OPENROUTER_API_KEY ||
        import.meta.env.OPENROUTER_API_KEY
      : process.env.REACT_APP_OPENROUTER_KEY || process.env.OPENROUTER_API_KEY;

  if (!API_KEY) {
    console.error(
      "Missing API key. Expected REACT_APP_OPENROUTER_KEY in environment variables."
    );
    throw new Error(
      "API service not configured - check your environment variables"
    );
  }
