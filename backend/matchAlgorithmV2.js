...
function calculateMatchScore(userA, userB) {
  ...
  const courseOverlap = userA.courses?.some((course) =>
...
function matchUsersV2(currentUser, allUsers, excludeUserIds = []) {
  ...
  console.log(
    `User ${currentUser.id} (reputation: ${myReputation}) - Found ${reputationFilteredUsers.length} users within reputation band [${minReputation}-${maxReputation}] (excluding ${excludeUserIds.length} connected users)`
  );
  ...
  for (const otherUser of reputationFilteredUsers) {
    ...
    if (reputationFilteredUsers.indexOf(otherUser) < 3) {
      ...
      console.log(
        `Debug - Other User (${otherUser.id}) Courses:`,
        otherUser.courses
      );
    }
...
async function runMatchingForUser(userId) {
  ...
  console.log(
    `User ${userId} has ${connectedUserIds.length} confirmed connections`
  );

  // Get all potential matches (excluding connected users)
  const allPotentialMatches = matchUsersV2(
    currentUser,
    allUsers,
    connectedUserIds
  );
  ...
  console.log(
    `Found ${allPotentialMatches.length} potential matches after filtering`
  );
